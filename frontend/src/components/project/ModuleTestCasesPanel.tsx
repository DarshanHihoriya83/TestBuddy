...
export function ModuleTestCasesPanel({
  ...
  async function runBulkTestCaseExport(format: RecordExportFormat, includeDetails: boolean) {
    const list = testCases.filter((tc) => selectedIds.has(tc.id));
    if (!list.length) {
      notifyError("No test cases selected to export");
      return;
    }
    const docs: ExportRecordDoc[] = list.map((tc) => ({
      entity: "Test Case",
      displayId: shortId(tc.id),
      title: tc.title,
      context: `Module: ${moduleName || "\u2014"}`,
      contents: [
        { label: "Steps", value: String(tc.steps?.length ?? 0) },
        {
          label: "Expected Results",
          value: String((tc.steps ?? []).filter((s) => s.expectedResult?.trim()).length),
        },
        { label: "Preconditions", value: tc.preconditions?.trim() ? "Yes" : "No" },
        { label: "Linked Bug", value: tc.linkedBugId ? "Yes" : "No" },
        { label: "Assigned", value: tc.assigneeId ? "Yes" : "No" },
        { label: "AI Draft", value: tc.generatedByAi ? "Yes" : "No" },
      ],
      summary: [
        { label: "Test Case ID", value: shortId(tc.id) },
        { label: "Title", value: tc.title },
        { label: "Type", value: tc.type },
        { label: "Priority", value: tc.priority },
        { label: "Execution Status", value: execLabel(tc.executionStatus) },
        { label: "Assignee", value: nameOf(tc.assigneeId) },
        { label: "Updated On", value: formatDate(tc.updatedAt) },
      ],
      details: [
        { label: "Module", value: moduleName || "\u2014" },
        { label: "Review Status", value: tc.status },
        { label: "Generated by AI", value: tc.generatedByAi ? "Yes" : "No" },
        { label: "Preconditions", value: tc.preconditions?.trim() || "\u2014" },
        { label: "Description", value: tc.flowDescription?.trim() || "\u2014" },
        { label: "Created On", value: formatDate(tc.createdAt) },
      ],
      sections: [
        {
          title: "Steps",
          columns: ["#", "Action", "Expected Result"],
          rows: (tc.steps ?? []).map((s, i) => [
            String(s.order ?? i + 1),
            s.action ?? "",
            s.expectedResult ?? "",
          ]),
    ...
    await exportRecords(format, docs, { includeDetails });
    notifySuccess(
      `Exported ${list.length} test case${list.length === 1 ? "" : "s"} as ${format.toUpperCase()}`,
    );
  }
    ...
    <div className="grid gap-3">
      <label className="tb-label">
        ...
        <input
          ...
          placeholder="e.g, Remember me keeps session after refresh"
          required
        />
      </label>
      <label className="tb-label">
  ...
  return (
    <div className={`tb-mod-panel flex min-h-0 flex-1 flex-col ${tableDensityClass(viewPrefs)}`}>
      ...
      <BulkExportModal
        ...
        entitySingular="Test Case"
        selectedCount={selectedIds.size}
        detailsLabel="Test Case Details"
        detailsHint="Includes steps, expected results, preconditions and more for each case."
        onClose={() => setBulkExportOpen(false)}
        onExport={runBulkTestCaseExport}
      />
