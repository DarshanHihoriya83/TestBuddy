...
export async function createProject(actor, { name, organizationId, jiraProjectKey, adoOrgUrl, adoProject }) {
  ...
  return withTransaction(async (client) => {
    ...
    const { rows } = await client.query(
      `INSERT INTO projects (id, name, jira_project_key, ado_org_url, ado_project, organization_id, created_by)
...
export async function updateProject(actor, id, { name, jiraProjectKey, adoOrgUrl, adoProject }) {
  ...
  const { rows } = await query(
    `UPDATE projects
     ...
     WHERE id = $5
     RETURNING *`,
    [projectName, blankToNull(jiraProjectKey), blankToNull(adoOrgUrl), blankToNull(adoProject), id],
  );
  return toProjectDto(rows[0]);
}
